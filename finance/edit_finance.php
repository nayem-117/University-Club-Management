...
$stmt = $conn->prepare("SELECT * FROM finance WHERE finance_id = ?");
...
if (isset($_POST['submit'])) {
    ...
    if (!$error) {
        $stmtUpdate = $conn->prepare("UPDATE finance SET event_id = ?, type = ?, amount = ?, description = ?, transaction_date = ?, updated_at = NOW() WHERE finance_id = ?");
        $stmtUpdate->bind_param("isdssi", $event_id, $type, $amount, $description, $transaction_date, $finance_id);
        if ($stmtUpdate->execute()) {
            $success = "Finance record updated successfully.";
            $finance = ['event_id' => $event_id, 'type' => $type, 'amount' => $amount, 'description' => $description, 'transaction_date' => $transaction_date];
        } else {
            $error = "Failed to update record: " . $stmtUpdate->error;
        }
        $stmtUpdate->close();
    }
} else {
...
<title>Edit Finance Record - University Club Management</title>
...
<div class="bg-white rounded shadow-xl w-full max-w-3xl p-8 ring-4 ring-teal-300">
    ...
    <h1 class="text-3xl font-bold text-teal-700 mb-6 text-center drop-shadow">Edit Finance Record</h1>
    ...
    <form method="POST" class="space-y-6" novalidate>
        <div>
            ...
            <select name="event_id" id="event_id" required class="w-full p-3 border rounded border-gray-300 focus:ring-4 focus:ring-teal-400">
                ...
                <?php foreach ($events as $ev): ?>
                    <option value="<?= htmlspecialchars($ev['event_id']) ?>" <?= ($event_id == $ev['event_id']) ? 'selected' : '' ?>>
        ...
        <div class="flex justify-between items-center">
            ...
            <a href="view_finance.php" class="text-teal-600 font-semibold hover:underline">Back to Finance Records</a>
        </div>
    </form>
