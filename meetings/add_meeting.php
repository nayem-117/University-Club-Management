...
if (isset($_POST['submit'])) {
    ...
    if (!$error) {
        $stmt = $conn->prepare("INSERT INTO meetings (meeting_date, agenda, decisions) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $meeting_date, $agenda, $decisions);
        if ($stmt->execute()) {
            $success = "Meeting added successfully.";
            $meeting_date = $agenda = $decisions = "";
        } else {
            $error = "Failed to add meeting: " . $stmt->error;
        }
        $stmt->close();
    }
}
...
<title>Add Meeting - University Club Management</title>
...
<div class="bg-white rounded shadow-xl w-full max-w-3xl p-8 ring-4 ring-indigo-300">
    <h1 class="text-3xl font-extrabold text-indigo-700 mb-8 text-center drop-shadow-lg">Add New Meeting</h1>
